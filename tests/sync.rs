...
async fn test_sync_events() -> anyhow::Result<()> {
    ...
    test.ctx_mut().seq.set_call_response(101).await;
    test.ctx_mut().eth.set_call_response(202).await;

    let ctx = test.ctx().clone();
    let src = Source::new(ctx);
    src.add("seq", sync::poll_seq, Duration::from_millis(300)).await;
    src.add("eth", sync::poll_eth, Duration::from_millis(600)).await;
    let mut src = src.run();
